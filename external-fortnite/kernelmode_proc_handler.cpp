...
bool kernelmode_proc_handler::attach(const char* proc_name) {
	...
	if (handle == INVALID_HANDLE_VALUE) { //GH2ST P2C Example by Happy Cat
		MessageBox(0, XorStr("Driver Failed contact ro#1337").c_str(), 0, 0);
		return false;
	}
...
void kernelmode_proc_handler::write_memory(uintptr_t dst, uintptr_t src, size_t size) {
	...
	DWORD bytes_read;
	k_rw_request request{ pid, src, dst, size };
	DeviceIoControl(handle, ioctl_write_memory, &request, sizeof(k_rw_request), 0, 0, &bytes_read, 0); //GH2ST P2C Example by Happy Cat
}
